...
public:
    ...
    void DrawFrame(GLFWwindow* window, Camera camera, std::vector<RenderObject>* renderObjects) {
        //vkWaitForFences(_device, 1, &inFlightFences[_currentFrame], VK_TRUE, UINT64_MAX);
        ...
        /*currentUniformBufferObject = UniformBufferObject{};
        currentUniformBufferObject.view = camera.GetViewMatrix();
        currentUniformBufferObject.proj = camera.GetProjectionMatrix();
        memcpy(uniformBuffersMapped[_currentFrame], &currentUniformBufferObject, sizeof(currentUniformBufferObject));*/
    ...
    void Cleanup() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            _cameraUniformBufferMemory[i].CleanUp(_device);
            _renderOjbectsDynamicUniformBufferMemory[i].CleanUp(_device);
        }
...
private:
   ...
   /* std::vector<VkBuffer> uniformBuffers;
    ...
    //UniformBufferObject currentUniformBufferObject;
    ...
    void createSwapChain(GLFWwindow* window) {
        ...
        if (indices.graphicsFamily != indices.presentFamily) {
            ...
            createInfo.queueFamilyIndexCount = static_cast<uint32_t>(deviceExtensions.size());
            createInfo.pQueueFamilyIndices = queueFamilyIndices;
        }
        else {
            createInfo.imageSharingMode = VK_SHARING_MODE_CONCURRENT;
            createInfo.queueFamilyIndexCount = static_cast<uint32_t>(deviceExtensions.size()) - 1; // Optional
            createInfo.pQueueFamilyIndices = queueFamilyIndices; // Optional
        }
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex, std::vector<RenderObject>* renderObjects) {
        ...
        //vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, _pipelineLayout, 0, 1, &_cameraDescriptorSets[_currentFrame], 1, 0);

        for (unsigned int i = 0; i < (*renderObjects).size(); i++) {
            //currentUniformBufferObject.model = (*renderObjects)[i].transform.getLocalToWorldMatrix();
            //glm::mat4 translation = glm::translate(glm::mat4(1), glm::vec3(1.0f, 0.0f, 0.0f));
            //glm::mat4 rotation = glm::rotate(glm::mat4(1), val * glm::radians(10.0f), glm::vec3(0.0f, 1.0f, 0.0f));
            //currentUniformBufferObject.model = translation * rotation;
            //memcpy(_renderOjbectsDynamicUniformBufferMemory[_currentFrame].uniformBuffersMapped, &currentUniformBufferObject, sizeof(currentUniformBufferObject));
            //memcpy(_renderOjbectsDynamicUniformBufferMemory[_currentFrame].uniformBuffersMapped, &(*renderObjects)[i].modelUniformBufferObject, sizeof(RenderObjectUniformBufferOjbect));
            ...
            //vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, _pipelineLayout, 0, 1, &_descriptorSets[_currentFrame], 1, 0);
            //vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, _pipelineLayout, 0, 1, &_objectDescriptorSets[_currentFrame], 1, 0);
    ...
    void createDescriptorPool() {
        ...
        VkDescriptorPoolSize poolSizes[] = { cameraPoolSize , renderObjectPoolSize };
    ...
    void createDescriptorSetLayout() {
        ...
        //modelLayoutInfo.pBindings = &modelUboLayoutBinding;

        //if (vkCreateDescriptorSetLayout(_device, &modelLayoutInfo, nullptr, &_descriptorSetObjectLayout) != VK_SUCCESS) {
        //    throw std::runtime_error("failed to create descriptor set ojbect layout!");
        //}
    }
    ...
    void createDescriptorSets() {
        ...
        /*std::vector<VkDescriptorSetLayout> objectLayouts(MAX_FRAMES_IN_FLIGHT, _descriptorSetObjectLayout);
        VkDescriptorSetAllocateInfo allocObjectInfo{};
        allocObjectInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
        allocObjectInfo.descriptorPool = _descriptorPool;
        allocObjectInfo.descriptorSetCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);
        allocObjectInfo.pSetLayouts = objectLayouts.data();

        VkResult result = vkAllocateDescriptorSets(_device, &allocObjectInfo, _objectDescriptorSets);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            objectBufferInfo.range = _renderOjbectsDynamicUniformBufferMemory[i].bufferSize;
            ...
            objectDescriptorWrite.dstBinding = 1;
            ...
            //vkUpdateDescriptorSets(_device, 1, &objectDescriptorWrite, 0, nullptr);

            /*cameraBufferInfo.buffer = _renderOjbectsDynamicUniformBufferMemory[i].uniformBuffer;
            cameraBufferInfo.offset = 0;
            cameraBufferInfo.range = sizeof(RenderObjectUniformBufferOjbect);*/
