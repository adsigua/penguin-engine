...
#pragma region Public
        ...
        void VKEngine::Cleanup() {
            ...
            _textureImage.DestroyAllocatedImage(_device, _allocator);
...
#pragma region SwapChain
        ...
        void VKEngine::createSwapChain() {
            ...
            for (int i = 0; i < imageCount; i++) {
                ...
                image.image = _swapChainImages[i];
                image.imageFormat = surfaceFormat.format;
                image.imageExtent = extent;

                SwapChainData swapChainData{};
                swapChainData.allocatedImage = image;
        ...
        void VKEngine::createSwapChainImageViews() {
            _swapChainImageViews.resize(_swapChainData.size());
            for (size_t i = 0; i < _swapChainData.size(); i++) {
                //_swapChainImageViews[i] = createImageView(_swapChainData[i].allocatedImage, _swapChainData[i].allocatedImage.imageFormat, VK_IMAGE_ASPECT_COLOR_BIT);
                _swapChainData[i].allocatedImage.imageView = createImageView(_swapChainData[i].allocatedImage, _swapChainData[i].allocatedImage.imageFormat, VK_IMAGE_ASPECT_COLOR_BIT);
            }
        }
...
#pragma region Buffers
        ...
        void VKEngine::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
...
#pragma region Descriptors
        ...
        void VKEngine::createDescriptorSets() {
            ...
            for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
                ...
                VkWriteDescriptorSet descriptorWriteSets[] = { cameraDescriptorWrite , objectDescriptorWrite };
                vkUpdateDescriptorSets(_device, 2, descriptorWriteSets, 0, nullptr);
            }
        }
#pragma endregion
...
#pragma region Textures
        void VKEngine::createTextureImage() {
            ...
            if (vmaCreateBuffer(_allocator, &stagingBufInfo, &stagingBufAllocCreateInfo, &stagingBuffer, &stagingBufAlloc, &stagingBufAllocInfo) != VK_SUCCESS) {
                throw std::runtime_error("failed to create buffer!");
            }

            memcpy(stagingBufAllocInfo.pMappedData, pixels, static_cast<size_t>(imageSize));

            stbi_image_free(pixels);

            createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &_textureImage);
            transitionImageLayout(_textureImage.image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);

            copyBufferToImage(stagingBuffer, _textureImage.image, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));

            transitionImageLayout(_textureImage.image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

            vmaDestroyBuffer(_allocator, stagingBuffer, stagingBufAlloc);*/
        ...
        void VKEngine::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, AllocatedImage* allocatedImage) {
            //VkImageCreateInfo imageInfo = VKInit::CreateImageCreateInfo(width, height, format, tiling, usage);

            //VmaAllocationCreateInfo imgAllocInfo = {};
            //imgAllocInfo.usage = VMA_MEMORY_USAGE_GPU_ONLY;
            //imgAllocInfo.requiredFlags = VkMemoryPropertyFlags(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

            /*if (vmaCreateImage(_allocator, &imageInfo, &imgAllocInfo, &allocatedImage->image, &allocatedImage->allocation, nullptr) != VK_SUCCESS) {
            ...
            if (vkCreateImage(_device, &imageInfo, nullptr, &allocatedImage->image) != VK_SUCCESS) {
                throw std::runtime_error("failed to create image!");
            }

            VkMemoryRequirements memRequirements;
            vkGetImageMemoryRequirements(_device, allocatedImage->image, &memRequirements);
            ...
            if (vkAllocateMemory(_device, &allocInfo, nullptr, &allocatedImage->imageMemory) != VK_SUCCESS) {
                throw std::runtime_error("failed to allocate image memory!");
            }

            vkBindImageMemory(_device, allocatedImage->image, allocatedImage->imageMemory, 0);
        }
        ...
        VkImageView VKEngine::createImageView(AllocatedImage allocatedImage, VkFormat imageViewFormat, VkImageAspectFlags aspectFlags) {
            return createImageView(allocatedImage.image, imageViewFormat, aspectFlags);
        }
        ...
        void VKEngine::createTextureImageView() {
            _textureImage.imageView = createImageView(_textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
        }
        ...
        void VKEngine::createDepthResources() {
            ...
            createImage(_swapChainExtent.width, _swapChainExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &_depthTextureImage);
            _depthImageView = createImageView(_depthTextureImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);

            transitionImageLayout(_depthTextureImage.image, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
        }
